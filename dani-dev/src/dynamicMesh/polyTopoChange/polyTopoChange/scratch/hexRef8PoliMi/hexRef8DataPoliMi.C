...
Foam::hexRef8PoliMiData::hexRef8PoliMiData(const IOobject& io)
{
    {
        IOobject rio(io);
        ...
        if (haveFile)
        {
            Info<< "Reading hexRef8PoliMi data : " << rio.name() << endl;
...
Foam::hexRef8PoliMiData::hexRef8PoliMiData
(
    const IOobject& io,
    const hexRef8PoliMiData& data,
...
(
    ...
    const UPtrList<const hexRef8PoliMiData>& procDatas
)
{
...
void Foam::hexRef8PoliMiData::sync(const IOobject& io)
{
    const polyMesh& mesh = dynamic_cast<const polyMesh&>(io.db());
...
void Foam::hexRef8PoliMiData::updateMesh(const mapPolyMesh& map)
{
...
void Foam::hexRef8PoliMiData::distribute(const mapDistributePolyMesh& map)
{
    if (cellLevelPtr_.valid())
    {
...
bool Foam::hexRef8PoliMiData::write() const
{
    bool ok = true;
    if (cellLevelPtr_.valid())
    {
