...
namespace Foam
{
    defineTypeNameAndDebug(hexRef8PoliMi, 0);
...
void Foam::hexRef8PoliMi::reorder
(
    const labelList& map,
    const label len,
    const label null,
...
void Foam::hexRef8PoliMi::getFaceInfo
(
    const label facei,
    label& patchID,
    label& zoneID,
...
Foam::label Foam::hexRef8PoliMi::addFace
(
...
// for the face)
Foam::label Foam::hexRef8PoliMi::addInternalFace
(
    polyTopoChange& meshMod,
...
void Foam::hexRef8PoliMi::modFace
(
    polyTopoChange& meshMod,
    const label facei,
    const face& newFace,
...
Foam::scalar Foam::hexRef8PoliMi::getLevel0EdgeLength() const
{
    ...
    if (debug)
    {
        Pout<< "hexRef8PoliMi::getLevel0EdgeLength() :"
...
Foam::label Foam::hexRef8PoliMi::getAnchorCell
(
...
void Foam::hexRef8PoliMi::getFaceNeighbours
(
    const labelListList& cellAnchorPoints,
    const labelListList& cellAddedCells,
    const label facei,
...
Foam::label Foam::hexRef8PoliMi::findMinLevel(const labelList& f) const
{
...
Foam::label Foam::hexRef8PoliMi::findMaxLevel(const labelList& f) const
{
...
Foam::label Foam::hexRef8PoliMi::countAnchors
(
    const labelList& f,
...
void Foam::hexRef8PoliMi::dumpCell(const label celli) const
{
    OFstream str(mesh_.time().path()/"cell_" + Foam::name(celli) + ".obj");
    Pout<< "hexRef8PoliMi : Dumping cell as obj to " << str.name() << endl;
...
Foam::label Foam::hexRef8PoliMi::findLevel
(
    const label facei,
    const face& f,
    const label startFp,
...
Foam::label Foam::hexRef8PoliMi::faceLevel(const label facei) const
{
    const face& f = mesh_.faces()[facei];
...
void Foam::hexRef8PoliMi::checkInternalOrientation
(
    polyTopoChange& meshMod,
    const label celli,
    const label facei,
...
void Foam::hexRef8PoliMi::checkBoundaryOrientation
(
    polyTopoChange& meshMod,
    const label celli,
    const label facei,
...
// splitPoint.
void Foam::hexRef8PoliMi::insertEdgeSplit
(
...
Foam::label Foam::hexRef8PoliMi::storeMidPointInfo
(
    const labelListList& cellAnchorPoints,
    const labelListList& cellAddedCells,
...
void Foam::hexRef8PoliMi::createInternalFaces
(
    const labelListList& cellAnchorPoints,
    const labelListList& cellAddedCells,
    const labelList& cellMidPoint,
...
void Foam::hexRef8PoliMi::walkFaceToMid
(
    const labelList& edgeMidPoint,
    const label cLevel,
    const label facei,
...
void Foam::hexRef8PoliMi::walkFaceFromMid
(
    const labelList& edgeMidPoint,
    const label cLevel,
    const label facei,
...
Foam::label Foam::hexRef8PoliMi::faceConsistentRefinement
(
    const bool maxSet,
...
void Foam::hexRef8PoliMi::checkWantedRefinementLevels
(
...
void Foam::hexRef8PoliMi::setInstance(const fileName& inst)
{
    if (debug)
    {
        Pout<< "hexRef8PoliMi::setInstance(const fileName& inst) : "
...
void Foam::hexRef8PoliMi::collectLevelPoints
(
    const labelList& f,
    const label level,
...
bool Foam::hexRef8PoliMi::matchHexShape
(
    const label celli,
...
Foam::hexRef8PoliMi::hexRef8PoliMi(const polyMesh& mesh, const bool readHistory)
:
    mesh_(mesh),
    cellLevel_
    (
...
Foam::hexRef8PoliMi::hexRef8PoliMi
(
    const polyMesh& mesh,
...
Foam::labelList Foam::hexRef8PoliMi::consistentRefinement
(
    const labelList& cellsToRefine,
    const bool maxSet
) const
{
    ...
    while (true)
    {
        ...
        if (debug)
        {
            Pout<< "hexRef8PoliMi::consistentRefinement : Changed " << nChanged
...
Foam::labelList Foam::hexRef8PoliMi::consistentSlowRefinement
(
...
) const
{
    ...
    while (true)
    {
        if (debug)
        {
            Pout<< "hexRef8PoliMi::consistentSlowRefinement : Seeded "
    ...
    if (debug)
    {
        Pout<< "hexRef8PoliMi::consistentSlowRefinement : From "
...
Foam::labelList Foam::hexRef8PoliMi::consistentSlowRefinement2
(
    const label maxFaceDiff,
    const labelList& cellsToRefine,
    const labelList& facesToCheck
) const
{
    ...
    while (true)
    {
        ...
        if (debug)
        {
            Pout<< "hexRef8PoliMi::consistentSlowRefinement2 : Changed " << nChanged
                << " refinement levels due to 2:1 conflicts."
                << endl;
        }
...
//        if (refineCell.get(celli))
    ...
    if (debug)
    {
        Pout<< "hexRef8PoliMi::consistentSlowRefinement2 : From "
            << cellsToRefine.size() << " to " << newCellsToRefine.size()
            << " cells to refine." << endl;
...

        {
            ...
            Pout<< "hexRef8PoliMi::consistentSlowRefinement2 : writing "
...
Foam::labelListList Foam::hexRef8PoliMi::setRefinement
(
...
)
{
    if (debug)
    {
        Pout<< "hexRef8PoliMi::setRefinement :"
    ...
    if (debug)
    {
        ...
        Pout<< "hexRef8PoliMi::setRefinement : Dumping " << splitCells.size()
    ...
    if (debug)
    {
        ...
        Pout<< "hexRef8PoliMi::setRefinement : Dumping " << splitFaces.size()
    ...
    if (debug)
    {
        Pout<< "hexRef8PoliMi::setRefinement : Splitting faces" << endl;
    }
...
void Foam::hexRef8PoliMi::storeData
(
...
void Foam::hexRef8PoliMi::updateMesh(const mapPolyMesh& map)
{
...
// sure the pointLevel_ and cellLevel_ are the size of the new mesh so we
// only need to account for reordering.
void Foam::hexRef8PoliMi::updateMesh
(
...
)
{
...

    {
        ...
        {
            // Assume it is after hexRef8PoliMi that this routine is called.
...
void Foam::hexRef8PoliMi::subset
(
...
void Foam::hexRef8PoliMi::distribute(const mapDistributePolyMesh& map)
{
    if (debug)
    {
        Pout<< "hexRef8PoliMi::distribute :"
            << " Updating various lists"
            << endl;
    }
...
void Foam::hexRef8PoliMi::checkMesh() const
{
    const scalar smallDim = 1e-6 * mesh_.bounds().mag();

    if (debug)
    {
        Pout<< "hexRef8PoliMi::checkMesh : Using matching tolerance smallDim:"
            << smallDim << endl;
    }
    ...
    if (debug)
    {
        Pout<< "hexRef8PoliMi::checkMesh : Returning" << endl;
    }
}


void Foam::hexRef8PoliMi::checkRefinementLevels
(
    const label maxPointDiff,
    const labelList& pointsToCheck
) const
{
    if (debug)
    {
        Pout<< "hexRef8PoliMi::checkRefinementLevels :"
...
const Foam::cellShapeList& Foam::hexRef8PoliMi::cellShapes() const
{
    if (cellShapesPtr_.empty())
    {
        if (debug)
        {
            Pout<< "hexRef8PoliMi::cellShapes() : calculating splitHex cellShapes."
                << " cellLevel:" << cellLevel_.size()
                << " pointLevel:" << pointLevel_.size()
                << endl;
        }
...
Foam::labelList Foam::hexRef8PoliMi::getSplitPoints() const
{
    if (debug)
    {
        checkRefinementLevels(-1, labelList(0));
    ...
    if (debug)
    {
        Pout<< "hexRef8PoliMi::getSplitPoints :"
...
//void Foam::hexRef8PoliMi::markIndex
//(
...
//void Foam::hexRef8PoliMi::markCellClusters
//(
//    const label maxLevel,
//    labelList& cluster
//) const
...
Foam::labelList Foam::hexRef8PoliMi::consistentUnrefinement
(
...
) const
{
    if (debug)
    {
        Pout<< "hexRef8PoliMi::consistentUnrefinement :"
...
void Foam::hexRef8PoliMi::setUnrefinement
(
...
)
{
    ...
    if (debug)
    {
        Pout<< "hexRef8PoliMi::setUnrefinement :"
            << " Checking initial mesh just to make sure" << endl;
        ...
        Pout<< "hexRef8PoliMi::setRefinement : Dumping " << pSet.size()
            << " points and "
...
bool Foam::hexRef8PoliMi::write(const bool write) const
{
    bool writeOk =
