...
#include "dynamicRefineFvMeshPoliMi.H"
...
#include "SortableList.H"// added for tavazzi's code
#include "Pstream.H"
...
namespace Foam
{
    defineTypeNameAndDebug(dynamicRefineFvMeshPoliMi, 0);
    addToRunTimeSelectionTable(dynamicFvMesh, dynamicRefineFvMeshPoliMi, IOobject);
}

// * * * * * * * * * * * * Protected Member Functions  * * * * * * * * * * * //

Foam::label Foam::dynamicRefineFvMeshPoliMi::count
(
...
void Foam::dynamicRefineFvMeshPoliMi::calculateProtectedCells
(
...
void Foam::dynamicRefineFvMeshPoliMi::readDict()
{
...
Foam::dynamicRefineFvMeshPoliMi::refine
(
...
Foam::autoPtr<Foam::mapPolyMesh>
Foam::dynamicRefineFvMeshPoliMi::unrefine
(
    const labelList& splitPoints // CHANGED
)
{
    // splitItem will correspond to splitPoints on 3D and splitEdges on 2D

    polyTopoChange meshMod(*this);
    
    // Play refinement commands into mesh changer.
    meshCutter_.setUnrefinement(splitPoints, meshMod); // CHANGED

    Map<label> faceToSplitPoint(2*splitPoints.size());

    bool bidimensional = false;
    for(int patchi = 0; patchi < meshCutter_.mesh().boundaryMesh().size(); patchi ++)
    {
        if (meshCutter_.mesh().boundaryMesh()[patchi].type()=="empty" ||
            meshCutter_.mesh().boundaryMesh()[patchi].type()=="wedge" )
        {
            bidimensional = true;
            break;
        }
    ...
    if(bidimensional)
    {
        Info << "inside unrefine function for 2D case" << endl;
        // TODO
        // ADD HERE THE LOGIC FOR THE 2D CASE
        ...
        // Look for master and slave patches.
        //label masterPatch = -1;
        label slavePatch = -1;
        ...
        forAll(splitPoints, i)
        {
            ...
            // this is a point on the master patch, i.e. the first appearing
            // empty/wedge patch
            // we can add this points directly
            ...
            // now, we have to add the ones on the slave patch
            const labelList& pEdges = pointEdges()[pointi];

            forAll(pEdges, pEdgei)
            {
                label otherPointi = edges()[pEdges[pEdgei]].otherVertex(pointi);
                
                const labelList& otherPointFaces = pointFaces()[otherPointi];
                forAll(otherPointFaces, otherPointFacei)
                {
                    label otherFace = otherPointFaces[otherPointFacei];
                    label otherPatch = meshCutter_.mesh().boundaryMesh().whichPatch(otherFace);
                    if(otherPatch != slavePatch)
                    {
                        // otherPoint is not on the slave patch
                        // Look for next edge
                        break;
                    }
                    else
                    {
                        // otherPoint is the one we want. We can add its faces
                        faceToSplitPoint.insert(otherFace, otherPointi);
                    }
                }
            }
        }
    ...
    else
    {
        ...
        // for each face resulting of split of face into four store the
        // midpoint
        faceToSplitPoint.resize(3*splitPoints.size());
...
Foam::scalarField
Foam::dynamicRefineFvMeshPoliMi::maxPointField(const scalarField& pFld) const
{
...
Foam::scalarField
Foam::dynamicRefineFvMeshPoliMi::maxCellField(const volScalarField& vFld) const
{// TODO he has done the min instead of the max
...
Foam::scalarField
Foam::dynamicRefineFvMeshPoliMi::cellToPoint(const scalarField& vFld) const
{
...
Foam::scalarField Foam::dynamicRefineFvMeshPoliMi::error
(
...
void Foam::dynamicRefineFvMeshPoliMi::selectRefineCandidates
(
    const scalar lowerRefineLevel,
    const scalar upperRefineLevel,
    const scalarField& vFld,
    PackedBoolList& candidateCell,
    //additional list --> sorting candidateCell based on cellError
...
) const
{
    ...
    // Create lists of the lists with size equal to the number of processors.
    scalarListList gatheredError(Pstream::nProcs());

    // Populate and gather the cellError values onto the master processor.
    ...
    // Collect elements and sort onto ALL processors
    allCellError =
                ListListOps::combine<scalarList>//"Combines sublists into one big list"
                (
    ...
    allCellError.reverseSort();//"Reverse (stable) sort the list"

}


Foam::labelList Foam::dynamicRefineFvMeshPoliMi::selectRefineCells
(
    const label maxCells,
    const label maxRefinement,
    const PackedBoolList& candidateCell,
    // list to account sorting by error
    const SortableList<scalar>& allCellError
) const
{

    // check if 2D or 3D case
    // CHANGED
    ...
    label nTotToRefine = -1;
    // Every refined cell causes 7/3 extra cells in 3D/2D
    if (bidimensional)
    {
        nTotToRefine = (maxCells - globalData().nTotalCells()) / 3;
    }
    else
    {
        nTotToRefine = (maxCells - globalData().nTotalCells()) / 7;
    }
...
Foam::labelList Foam::dynamicRefineFvMeshPoliMi::selectUnrefinePoints
(
    // original 3D function
...
/*
Foam::labelList Foam::dynamicRefineFvMeshPoliMi::selectUnrefineEdges
(
    // new 2D function
    // CHANGED
    const scalar unrefineLevel,
...
{
    // All edges that can be unrefined
...
void Foam::dynamicRefineFvMeshPoliMi::extendMarkedCells
(
...
void Foam::dynamicRefineFvMeshPoliMi::checkEightAnchorPoints
(
...
Foam::dynamicRefineFvMeshPoliMi::dynamicRefineFvMeshPoliMi(const IOobject& io)
:
    dynamicFvMesh(io),
    meshCutter_(*this),
    dumpLevel_(false),
...
Foam::dynamicRefineFvMeshPoliMi::~dynamicRefineFvMeshPoliMi()
{}


// * * * * * * * * * * * * * * * Member Functions  * * * * * * * * * * * * * //

bool Foam::dynamicRefineFvMeshPoliMi::update()
{
	...
	// Cells ordered based on cellError value
	SortableList<scalar> sortedError;
...

        {
           ...
           /* else
            {
                // 2D case
                labelList edgesToUnrefine// CHANGED
                (
                    selectUnrefineEdges
                    (
                        unrefineLevel,
                        refineCell,
                        maxCellField(vFld) // Luca put min
                    )
                );
...
bool Foam::dynamicRefineFvMeshPoliMi::writeObject
(
...
) const
{
    ...
    const_cast<hexRefPoliMi&>(meshCutter_).setInstance(time().timeName());
